...
map_testfile() ->
    {module, ?MISC:canonical_filename("map_test.erl"),
     "-module(map_test).\n"
     "init() -> #{a => 1}.\n"
     "f() -> A = init(),\n"
     "       A#{b => 2}.\n"
...
map_test_()->
    {setup,
     fun() -> add_file(map_testfile()) end,
     fun(File) -> ?FileMan:drop_file(File) end,
     map_tests()}.

map_tests() ->
    Filename = ?MISC:canonical_filename("map_test.erl"),
    [?_assertMatch([{group_by,{nopos, _MapName},
		     list,
		     [{{Filename,30,38},"#{a => 1}"}]}],
		   refusr_sq:run(
		    [{positions, scalar},{output, other}],
     ...
     ?_assertMatch([{group_by,{nopos, _MapName},
		     ...
		     [{{Filename,48,48},"A"},
		      {{Filename,67,67},"A"},
		      {{Filename,67,76},"A#{b => 2}"},
		      {{Filename,86,86},"B"},
		      {{Filename,105,105},"B"},
		      {{Filename,105,114},"B#{a => 3}"}]}],
		   refusr_sq:run(
		    [{positions, scalar},{output, other}],
		    [{file, Filename}, {position, 34}],
		    "mods.funs.maps.ref")),
     ?_assertMatch([{group_by,{nopos, _MapName},
		     list,
		     [{{Filename,20,39},"map_test:init/0"}]}],
		   refusr_sq:run(
		    [{positions, scalar},{output, other}],
		    [{file, Filename}, {position, 34}],
		    "mods.funs.maps.func")),
     ?_assertMatch([{group_by,{nopos, _MapName},
		     list,
		     [{{Filename,20,39}, _Key1},
		      {{Filename,20,39}, _Key2}]}],
		   refusr_sq:run(
		    [{positions, scalar},{output, other}],
		    [{file, Filename}, {position, 34}],
		    "mods.funs.maps.keys")),
     ?_assertEqual([{group_by,{nopos,"#{a => 1}"},
		     list,
		     [{{Filename,20,39},"map_test:init/0"}]}],
		   refusr_sq:run(
		    [{positions, scalar},{output, other}],
		    [{file, Filename}, {position, 34}],
		    "mods.funs.maps.def.fundef"))].

mapkey_testfile() ->
    {module, ?MISC:canonical_filename("mapkey_test.erl"),
     "-module(mapkey_test).\n"
     "init() -> #{a => 1}.\n"
     "f() -> A = init(),\n"
     "       A#{b => 2}.\n"
...
mapkey_test_()->
    {setup,
     fun() -> add_file(mapkey_testfile()) end,
     fun(File) -> ?FileMan:drop_file(File) end,
     mapkey_tests()}.

mapkey_tests() ->
    Filename = ?MISC:canonical_filename("mapkey_test.erl"),
    [?_assertMatch([{group_by,{nopos, _Key1},
		     list,
		     [{{Filename,35,35},"a"}]},
		    {group_by,{nopos, _Key2},
		     list,
		     [{{Filename,73,73},"b"}]}],
		   refusr_sq:run(
		    [{positions, scalar},{output, other}],
		    [{file, Filename}, {position, 34}],
		    "mods.funs.maps.keys.def")),
     ?_assertMatch([{group_by,{nopos, _Key1},
		     list,
		     [{{Filename,111,111},"a"}]}],
		   refusr_sq:run(
		    [{positions, scalar},{output, other}],
		    [{file, Filename}, {position, 34}],
		    "mods.funs.maps.keys.ref")),
     ?_assertMatch([{group_by,{nopos, _Key1},
		     list,
		     [{{Filename,23,42}, _MapName}]},
		    {group_by,{nopos, _Key2},
		     list,
		     [{{Filename,23,42}, _MapName}]}],
		   refusr_sq:run(
		    [{positions, scalar},{output, other}],
		    [{file, Filename}, {position, 34}],
		    "mods.funs.maps.keys.maps")),
     ?_assertMatch([{group_by,{nopos, _Key1},
		     list,
		     [{{Filename,23,42}, "mapkey_test:init/0"}]},
		    {group_by,{nopos, _Key2},
		     list,
		     [{{Filename,23,42}, "mapkey_test:init/0"}]}],
		   refusr_sq:run(
