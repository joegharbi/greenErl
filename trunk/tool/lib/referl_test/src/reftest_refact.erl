...
% balassai form cache test
form_cache_test(Form) ->
   A = (?Graph:data(Form)),
   Cache = A#form.cache,
   Check = ?Syn:flat_text_legacy(Form),
   Cache =/= Check.

% balassai form cache tester
cache_tests() ->
    Files = ?Query:exec([file]),
    Forms = lists:flatten([?FileMan:real_forms(File) || File <- Files]),
    Result = [{err_form_cache_value_differ, Form, (?Graph:data(Form))#form.cache, ?Syn:flat_text_legacy(Form)} || Form <- Forms, form_cache_test(Form) ],
    Result.
