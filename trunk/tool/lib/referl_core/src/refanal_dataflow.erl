...
add_del(Dir, #expr{type=map_update}, Expr, _P) ->
    [{_,MapExp}, {_,KVPList}] = ?Anal:children(Expr),
    safe_link(Dir, MapExp, flow, Expr),
    [{MapExp, Expr}] ++ [{KVP, Expr} || {_,KVP} <- ?Anal:children(KVPList)];

add_del(_Dir, #expr{type=assoc_map_expr}, Expr, _P) ->
    [{_, KVPList}] = ?Anal:children(Expr),
    [{KVP, Expr} || {_,KVP} <- ?Anal:children(KVPList)];
