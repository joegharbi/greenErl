...
(define-refac-operation (refactorerl-upgrade-regexp-iface :menu "Upgrade regexp interface"
                                                          :menu-group upgrade
                                                          :key "uir"
                                                          :precondition (lambda () (and (not refactorerl-is-readonly) :buffer-state)))
  ((pos :point))
...
;; *****************************************************************************
;; Eliminate get_value
(define-refac-operation (refactorerl-eliminate-get-value :menu "Eliminate proplists:get_value" :key "eg"
                                                       :menu-group introelim
                                                       :precondition (lambda () (and (not refactorerl-is-readonly) :buffer-state)))
  ((pos :point))
  "Eliminates the call to proplists:get_value and replaces it with lists:key_find

1. Position the cursor over the name of the function to be eliminated.
2. Call the refactoring from the menu or with \\[refactorerl-eliminate-get-value]."
  (refac-transform 'elim_get_value
                   :file buffer-file-name
                   :position pos))

;; *****************************************************************************
;; Transform list to map
(define-refac-operation (refactorerl-transform-list-map :menu "Transform list to map" :key "tm"
                                                       :menu-group introelim
                                                       :precondition (lambda () (and (not refactorerl-is-readonly) :buffer-state)))
  ((pos :point))
  "Transforms the list data stucture to use a map instead
  
1. Position the cursor over the name of the variable to be transformed.
2. Call the refactoring from the menu or with \\[refactorerl-eliminate-get-value]."
  (refac-transform 'transform_list
                   :file buffer-file-name
                   :position pos))

;; *****************************************************************************
;; Eliminate higher order function
(define-refac-operation (refactorerl-eliminate-hof :menu "Eliminate higher order function" :key "eh"
                                                       :menu-group introelim
                                                       :precondition (lambda () (and (not refactorerl-is-readonly) :buffer-state)))
  ((pos :point))
  "Eliminates the higher order function using a list comprehension or
  a new recursive function
  
1. Position the cursor over the higher order function call.
2. Call the refactoring from the menu or with \\[refactorerl-eliminate-hof].
3. Select which method to use for elimination."
  (refac-transform 'elim_hof
                   :file buffer-file-name
                   :position pos))