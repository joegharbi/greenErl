...
-export([measure/4, detsToFile/1, detsToFiles/1]).
...
measure(Prog, {Module, Functions, [InputDesc|InputDescs]}, Count, ResultPath) -> 
    ...
    if
        IsInputDescList -> 
            IsGenerateInputAvailable = lists:member({generate_input,length(InputDesc)}, apply(Module, module_info, [exports])),
            if
                IsGenerateInputAvailable ->
                    ...
                    measureFunctions(Prog, {Module, Functions, ArgList, InputDesc}, Count, ResultPath);
                true ->
...
measureFunctions(Prog, {Module, Functions, Attributes=[H|_]}, Count, ResultPath) -> measureFunctions(Prog, {Module, Functions, Attributes, H}, Count, ResultPath);
...
measureFunctions(Prog, {Module, all, Attributes, InputDesc}, Count, ResultPath) ->
    Functions = lists:map(fun({FunctionName, _}) -> FunctionName end, lists:filter(fun({FunctionName, Arity}) -> Arity == length(Attributes) andalso FunctionName /= module_info andalso FunctionName /= generate_input end, apply(Module, module_info, [exports]))),   
    measureFunctions(Prog, {Module, Functions, Attributes, InputDesc}, Count, ResultPath);
measureFunctions(Prog, {Module, [Function|Functions], Attributes, InputDesc}, Count, ResultPath) ->
    ...
    measureFunction(Prog, {Module,Function,Attributes,InputDesc}, Count , ResultOutput, AvgOutput, LogFileName),
    measureFunctions(Prog, {Module, Functions, Attributes, InputDesc}, Count, ResultPath);
measureFunctions(_, {_, [], _, _}, _, _) -> ok.
...
measureFunction(Prog, {M,F,A=[H|_]}, 0, ResultOutput, AvgOutput, LogFileName) -> measureFunction(Prog,{M,F,A,H}, 0, ResultOutput, AvgOutput, LogFileName);
measureFunction(Prog, {M,F,A=[H|_]}, Count , ResultOutput, AvgOutput, LogFileName) -> measureFunction(Prog, {M,F,A,H}, Count , ResultOutput, AvgOutput, LogFileName);
    ...
    receive
        {Port, {data, [1]}} -> %Rapl-read is ready to initialize measurement
            ...
            try test1(Port,{M,F,A,O},Output) of
                ...
                throw:no_init -> io:format("Failed: not ready initializing~n");
                throw:not_received -> io:format("Failed: measurement not received~n")
            end;
        X ->
            io:format("~p~n", [X])
    after 5000 ->
...
receive_measurement(Port,FileName,Input={M,F,_,O},Time) -> 
    ...
    receive
        {Port, {data, [N]}} -> receive_measurement_method(Port,FileName,Input,N) %Receive number of measurements
    after 5000  ->
        throw(not_received)
    end,
    dets:close(FileName).

receive_measurement_method(_,_,_,0) -> ok;
...
receive_measurement_type(_,_,_,_,0) -> ok;
receive_measurement_type(Port,FileName,Input,Method,N) ->
    receive
...
receive_measurement_value(Port,FileName,{M,F,_,O},Method,Case) ->
    receive
        {Port, {data, Value}} ->  %Receive value of measurement
            <<X/float>> = erlang:list_to_binary(lists:reverse(Value)),
