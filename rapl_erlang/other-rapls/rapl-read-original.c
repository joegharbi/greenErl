...
double result_energy_rapl_perf[4];
	...
	for(j=0;j<total_packages;j++) {
		...
		for(i=0;i<NUM_RAPL_DOMAINS;i++) {
			if (valid[j][i]) {
				...
				printf("diference \t\t%s\t: %lfJ",event_names[j][i],((double)after[j][i]-(double)before[j][i])/1000000.0);
				temp = (((double)after[j][i]-(double)before[j][i])/1000000.0);
				printf("temp---> %lf \n", temp);
				result_energy[k]=temp;
...
int main(int argc, char **argv)
{
	...
	fprintf(fp,"cpu_model =  %i \n",cpu_model);
	fprintf(fp,"force_msr = %i \n",force_msr);
	fprintf(fp,"force_perf_event = %i \n",force_perf_event);
	fprintf(fp,"force_perf_event = %i \n",force_perf_event);
	fclose(fp);
	sleep(3);
	...
	{	
		...
		while((len=read_cmd(buf, sizeof(buf)))>0)
		{
			...
			if (n==4)
			{
				res_energy_send=rapl_sysfs_after(core);
				res_energy_rapl_perf=rapl_perf_after(core);
				fprintf(fp,"\n---Measuring with sysfs powercap interface---\n\n");

				for (j=0; j < 4 ; j++)
				{
					fprintf(fp,"%s: ~",name_events[j]);
					fprintf(fp,"%lf~ Joules \n",res_energy_send[j]);	
				}
				if(flag==2)
				{
					fprintf(fp,"\n-----Measuring with perf_event interface-----\n\n");
					for (j=0; j < 4 ; j++)
					{
						fprintf(fp,"%s: ~",rapl_domain_names[j]);
						fprintf(fp,"%lf~ Joules \n",res_energy_rapl_perf[j]);
					}
				}
				rapl_sysfs_before(core);
				rapl_perf_before(core);
