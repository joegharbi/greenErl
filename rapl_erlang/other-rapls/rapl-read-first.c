...
double msr_results[MAX_PACKAGES][5];
...
static int rapl_msr_before(int core, int cpu_model) {
	...
	for(j=0;j<total_packages;j++) {
		...
		package_before[j]=(double)result_msr*cpu_energy_units[j];
		...
		if (pp0_avail) {
			...
			pp0_before[j]=(double)result_msr*cpu_energy_units[j];
		}
		...
		if (pp1_avail) {
			...
			pp1_before[j]=(double)result_msr*cpu_energy_units[j];
		}
		...
		if (dram_avail) {
			...
			dram_before[j]=(double)result_msr*dram_energy_units[j];
		}
		...
		if ((cpu_model==CPU_SKYLAKE) ||
			...
			psys_before[j]=(double)result_msr*cpu_energy_units[j];
		}

		close(fd_msr);
	}
	fprintf(stderr,"%f",package_before[0]);
}
...
static int rapl_msr_after(int core, int cpu_model) {
	...
	for(j=0;j<total_packages;j++) {
		...
		result_msr=read_msr(fd_msr,MSR_PKG_ENERGY_STATUS);
		package_after[j]=(double)result_msr*cpu_energy_units[j];
		msr_results[j][0] = package_after[j]-package_before[j];

		result_msr=read_msr(fd_msr,MSR_PP0_ENERGY_STATUS);
		pp0_after[j]=(double)result_msr*cpu_energy_units[j];
		msr_results[j][1] = pp0_after[j]-pp0_before[j];

		/* not available on SandyBridge-EP */
		if (pp1_avail) {
			result_msr=read_msr(fd_msr,MSR_PP1_ENERGY_STATUS);
			pp1_after[j]=(double)result_msr*cpu_energy_units[j];
		...
		if (dram_avail) {
			result_msr=read_msr(fd_msr,MSR_DRAM_ENERGY_STATUS);
			dram_after[j]=(double)result_msr*dram_energy_units[j];
		...
		if (psys_avail) {
			result_msr=read_msr(fd_msr,MSR_PLATFORM_ENERGY_STATUS);
			psys_after[j]=(double)result_msr*cpu_energy_units[j];
	...
	for(j=0;j<total_packages;j++) {
		...
		for(i=0;i<NUM_RAPL_DOMAINS;i++) {
			...
			if (fd_perf[i][j]!=-1) {
				...
				// 	units[i]);
				result_energy_rapl_perf[k]=value*scale[i];
				k=k+1;
	...
	for(j=0;j<total_packages;j++) {
		for(i=0;i<NUM_RAPL_DOMAINS;i++) {
			if (valid[j][i]) {
				...
				result_energy[k]=(((double)after[j][i]-(double)before[j][i])/1000000.0);
				name_events[k]=event_names[j][i];
				k=k+1;
			}
		}
	}
		...
		while(read_cmd(buf)>0)
		{
			...
			if (n==2) //after
			{
				...
				for (j=0; j < 4 ; j++)
				{
					...
					if(force_msr)fprintf(fp,"%lf",msr_results[0][j]);
					fprintf(fp,"\n");
				}
				fprintf(fp,"\n");
				...
				if(force_msr){
					...
					for(j = 3; j>=0; --j){
						...
						memcpy(data,&msr_results[0][j], sizeof msr_results[0][j]);
						write_cmd(data, sizeof msr_results[0][j]);
					}
				}
