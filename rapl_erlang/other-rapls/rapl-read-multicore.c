...
static int package_map[MAX_PACKAGES]; //stores how many cpus are on given package;
static int package_cores[MAX_PACKAGES][MAX_CPUS]; //stores the id-s of the cpus on given package, the i-th row stores package_map[i] ids;
...
static int detect_packages(void) {
	...
	for(i=0;i<MAX_CPUS;i++) {
		...
		if (package_map[package]==0) {
			total_packages++;
		}
		package_cores[package][package_map[package]] = i;
		package_map[package]++;
	...
	// for(i = 0; i<MAX_PACKAGES; ++i){
	// 	fprintf(stderr,"package %d has %d cores with ids: \n",i,package_map[i]);
	// 	for(j = 0; j<package_map[i]; ++j){
	// 		fprintf(stderr,"%d, ",package_cores[i][j]);
	// 	}
	// 	fprintf(stderr,"\n");
	// }
...
double cpu_energy_units[MAX_PACKAGES][MAX_CPUS],dram_energy_units[MAX_PACKAGES][MAX_CPUS];
...
static int rapl_msr_before(int core, int cpu_model) {
	...
	for(j=0;j<total_packages;j++) {
		...
		for(k = 0; k<package_map[j]; ++k){
			...
			//fprintf(stderr,"packege %d cpu %d BEFORE %d\n\r",j,k,result_msr);
			package_before+=(double)result_msr*cpu_energy_units[j][k];
			...
			if (pp0_avail) {
				...
				pp0_before+=(double)result_msr*cpu_energy_units[j][k];
			}
			...
			if (pp1_avail) {
				...
				pp1_before+=(double)result_msr*cpu_energy_units[j][k];
			}
			...
			if (dram_avail) {
				...
				dram_before+=(double)result_msr*dram_energy_units[j][k];
			}
			...
			if ((cpu_model==CPU_SKYLAKE) ||
				...
				psys_before+=(double)result_msr*cpu_energy_units[j][k];
			}
...
static int rapl_msr_after(int core, int cpu_model) {
	...
	for(j=0;j<total_packages;j++) {
		...
		for(k = 0; k<package_map[j]; ++k){
			...
			//printf("\tPackage %d:\n",j);
			result_msr=read_msr(fd_msr,MSR_PKG_ENERGY_STATUS);
			fprintf(stderr,"packege %d cpu %d AFTER %d\n\r",j,k,result_msr);
			package_after+=(double)result_msr*cpu_energy_units[j][k];
			//msr_results[j][0] = package_after[j]-package_before[j];

			result_msr=read_msr(fd_msr,MSR_PP0_ENERGY_STATUS);
			pp0_after+=(double)result_msr*cpu_energy_units[j][k];
			//msr_results[j][1] = pp0_after[j]-pp0_before[j];

			/* not available on SandyBridge-EP */
			if (pp1_avail) {
				result_msr=read_msr(fd_msr,MSR_PP1_ENERGY_STATUS);
				pp1_after+=(double)result_msr*cpu_energy_units[j][k];
				//msr_results[j][2] = pp1_after[j]-pp1_before[j];
			}
			//else msr_results[j][2] = 0;

			if (dram_avail) {
				result_msr=read_msr(fd_msr,MSR_DRAM_ENERGY_STATUS);
				dram_after+=(double)result_msr*dram_energy_units[j][k];
				//msr_results[j][3] = dram_after[j]-dram_before[j];
			}
			//else msr_results[j][3] = 0;

			if (psys_avail) {
				result_msr=read_msr(fd_msr,MSR_PLATFORM_ENERGY_STATUS);
				psys_after+=(double)result_msr*cpu_energy_units[j][k];
				//msr_results[j][4] = psys_after[j]-psys_before[j];
			}
			//else msr_results[j][4] = 0;
...
int fd_perf[NUM_RAPL_DOMAINS][MAX_PACKAGES][MAX_CPUS];
long long value;
	...
	for(j=0;j<total_packages;j++) {
		...
		for(i=0;i<NUM_RAPL_DOMAINS;i++) {
			...
			for(k = 0; k<package_map[j]; ++k){
				...
				fd_perf[i][j][k]=perf_event_open(&attr,-1, package_cores[j][k],-1,0);
				//fprintf(stderr, "%d. package \"%s\" rapl domain %d. core %d fd_perf\n",j,rapl_domain_names[i],k,fd_perf[i][j][k]);
				if (fd_perf[i][j][k]<0) {
					if (errno==EACCES) {
...
static double* rapl_perf_after(int core) {
	...
	for(j=0;j<total_packages;j++) {
		...
		for(i=0;i<NUM_RAPL_DOMAINS;i++) {
			int l;
			for(l = 0; l<package_map[j]; ++l){
				if (fd_perf[i][j][l]!=-1) {
					read(fd_perf[i][j][l],&value,8);
					close(fd_perf[i][j][l]);
...
int main(int argc, char **argv)
{
	...
	{	
		...
		while(read_cmd(buf)>0)
		{
			...
			if (n==1) //before
			{
				if(force_sysfs)rapl_sysfs_before(core);
				if(force_perf)rapl_perf_before(core);
				if(force_msr)rapl_msr_before(core,cpu_model);
				buf[0] = 2;
				write_cmd(buf, 1);
			}
			if (n==2) //after
			{
				if(force_sysfs)res_energy_sysfs=rapl_sysfs_after(core);
				if(force_perf)res_energy_rapl_perf=rapl_perf_after(core);
				if(force_msr)rapl_msr_after(core,cpu_model);
				fprintf(fp,"\n%d. measurement\t",++i);
				if(force_sysfs)fprintf(fp,"sysfs");
				if(force_perf)fprintf(fp,"\tperf_event");
				if(force_msr)fprintf(fp,"\tMSR");
				fprintf(fp,"\n");
				for (j=0; j < 4 ; j++)
				{
					//fprintf(fp,"%s\t",name_events[j]);
					fprintf(fp,"%s\t",rapl_domain_names[j]);
					if(force_sysfs)fprintf(fp,"%lf\t",res_energy_sysfs[j]);
					if(force_perf)fprintf(fp,"%lf\t",res_energy_rapl_perf[j]);
					if(force_msr)fprintf(fp,"%lf",msr_results[j]);
					fprintf(fp,"\n");
				}
				fprintf(fp,"\n");
